...
- [x] **Matrix now fills the pane** — `CELL` size is computed from `#vlan-matrix-container.clientWidth` (was a fixed 44–60px band regardless of window size), capped at 110px for ≤20 VLANs (24–60px preserved above that); `#vlan-matrix-layout` gets `margin:0 auto` so small (few-VLAN) matrices are centered instead of stranded top-left
